...
def inference(protein_pdb, ligand_file, output, batch_size, reg_weight=DEFAULT_REG_WEIGHT, cls_weight=DEFAULT_CLS_WEIGHT, device='cpu', num_workers=0):
    dataset = PoseSelectionDataset(protein_pdb=protein_pdb, ligand_file=ligand_file)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='RMSD-Pred (PyG): Predict protein-ligand binding pose RMSD using Graph Neural Networks'
    )
    parser.add_argument('-r', '--protein_pdb', required=True, help='Receptor protein PDB file')
    parser.add_argument('-l', '--ligand_file', required=True, help='Ligand file (.sdf, .mol2, .dlg, .pdbqt, or .txt list)')
